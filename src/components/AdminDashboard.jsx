      ...
      >
        ...
        <div className="flex flex-col h-full">
          <div className="flex flex-col gap-4 px-4 lg:px-6 mt-6">
            <Link
              to="/admin"
              className={`flex gap-4 items-center py-2 px-4 w-full ${getLinkClassName('/admin')} h-12`}
            >
              <RiAdminLine className="h-8 w-8" />
              {isExpanded && <span className="text-lg lg:text-xl">Admin Dashboard</span>}
            </Link>
            <Link
              to="/teamsstudents"
              className={`flex gap-4 items-center py-2 px-4 w-full ${getLinkClassName('/teamsstudents')} h-12`}
            >
              <TiGroupOutline className="h-8 w-8" />
              {isExpanded && <span className="text-lg lg:text-xl">Teams & Students</span>}
            </Link>
            <Link
              to="/ourblogs"
              className={`flex gap-4 items-center py-2 px-4 w-full ${getLinkClassName('/ourblogs')} h-12`}
            >
              <TbBrandBlogger className="h-8 w-8" />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end">
          <div className="w-[300px] bg-white h-full p-4 flex flex-col">
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <RiAdminLine className="h-8 w-8" />
                <span className="text-lg lg:text-xl">Admin Dashboard</span>
              </Link>
              <Link
                to="/teamsstudents"
                className={`flex gap-4 items-center py-2 px-4 w-full ${getLinkClassName('/teamsstudents')} h-12`}
              >
                <TiGroupOutline className="h-8 w-8" />
                <span className="text-lg lg:text-xl">Teams & Students</span>
              </Link>
              <Link
                to="/ourblogs"
                className={`flex gap-4 items-center py-2 px-4 w-full ${getLinkClassName('/ourblogs')} h-12`}
